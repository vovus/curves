...
namespace DataLayer
{
	internal class SqlHelperSQLite
	{
		...
		/// </returns>
		internal static SQLiteCommand SelectCurrencyCommand(ConnectionContextSQLite ctx, long? idCurrency)
		{
			var command = new SQLiteCommand(null, ctx.connection, ctx.transaction);
#if _NO_VIEWS_
			...
			if (idCurrency != null)
			{
				sb.Append(" WHERE Id = @id AND UserCreated = @user");
		...
		/// <returns> MySqlCommand object
		/// </returns>
		internal static SQLiteCommand SelectYieldCurveFamilyCommand(ConnectionContextSQLite ctx, long? idYcFamily, long? idCcy)
		{
			var command = new SQLiteCommand(null, ctx.connection, ctx.transaction);
#if _NO_VIEWS_
			var sb = new StringBuilder("SELECT * FROM YcFamily WHERE 1 = 1 AND UserCreated = @user");
		...
		/// <returns> MySqlCommand object
		/// </returns>
		internal static SQLiteCommand SelectYieldCurveDataCommand(ConnectionContextSQLite ctx, long? idYc/*, long? idYcFamily*/)
		{
			var command = new SQLiteCommand(null, ctx.connection, ctx.transaction);
#if _NO_VIEWS_
		...
		/// <returns> MySqlCommand object
		/// </returns>
		internal static SQLiteCommand SelectYieldCurveEntryPointCommand(ConnectionContextSQLite ctx, long? idYc, DateTime? settlementDate)
		{
			var command = new SQLiteCommand(null, ctx.connection, ctx.transaction);
			var sb = new StringBuilder(
#if _NO_VIEWS_
			@"SELECT 
                    ...
                    WHERE 1 = 1 AND yce.UserCreated = @user"
				);
...
#endif
				p = new SQLiteParameter("@date", DbType.Date);
				p.Value = settlementDate;
				command.Parameters.Add(p);
			}
		...
		internal static SQLiteCommand SelectYieldCurveEntryPointHistoryCommand(ConnectionContextSQLite ctx,
																		   long? idYc,
																		   DateTime? settlementDate)
		{
			...
			sb.Append(" order by yce.YieldCurveId asc, yce.Id asc, yceh.Date desc");
		...
		internal static SQLiteCommand SelectRatesCommand(ConnectionContextSQLite ctx, long? idRate)
		{
			var command = new SQLiteCommand(null, ctx.connection, ctx.transaction);
			...
			WHERE 1 = 1 AND r.UserCreated = @user"
			);
			...
			if (idRate != null)
			{
				...
				p = new SQLiteParameter("@idRate", DbType.Int32);
				p.Value = idRate;
				command.Parameters.Add(p);
			}
    ...
    WHERE 1 = 1");
		...
		internal static SQLiteCommand SelectExchangeRateHistoryCommand(ConnectionContextSQLite ctx,
																	  long? RateId,
																	  DateTime? settlementDate)
		{
			var command = new SQLiteCommand(null, ctx.connection, ctx.transaction);
			...
			WHERE 1 = 1 AND xh.UserCreated = @user"
			);
		...
		internal static SQLiteCommand SelectInflationCurveEntryDataCommand(ConnectionContextSQLite ctx, long? idInflationCurve, DateTime? settlementDate)
		{
			var command = new SQLiteCommand(null, ctx.connection, ctx.transaction);
			var sb = new StringBuilder(
#if _NO_VIEWS_
			...
			WHERE 1 = 1 AND ice.UserCreated = @user"
			);
			...
			if (idInflationCurve != null)
			{
				...
				p = new SQLiteParameter("@idInflationCurve", DbType.Int32);
				p.Value = idInflationCurve;
				command.Parameters.Add(p);
			}
		...
		internal static SQLiteCommand SelectInflationBondsCommand(ConnectionContextSQLite ctx, long? idBond)
		{
			var command = new SQLiteCommand(null, ctx.connection, ctx.transaction);
			var sb = new StringBuilder(
#if _NO_VIEWS_
		...
		internal static SQLiteCommand SelectInflationIndexCommand(ConnectionContextSQLite ctx, long? idIndex)
		{
			var command = new SQLiteCommand(null, ctx.connection, ctx.transaction);
			...
			if (idIndex != null)
			{
				...
				p = new SQLiteParameter("@idIndex", DbType.Int32);
				p.Value = idIndex;
				command.Parameters.Add(p);
			}
		...
		internal static SQLiteCommand SelectInflationRatesCommand(ConnectionContextSQLite ctx, long? idRate)
		{
			var command = new SQLiteCommand(null, ctx.connection, ctx.transaction);
		...
		internal static SQLiteCommand SelectRateHistoryCommand(ConnectionContextSQLite ctx,
																	  long? RateId,
																	  DateTime? settlementDate)
		{
			var command = new SQLiteCommand(null, ctx.connection, ctx.transaction);
