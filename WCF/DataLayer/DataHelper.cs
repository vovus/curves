...
namespace DataLayer
{
    ...
    public class DataHelper
    {
		...
		internal static List<Currency> GetCurrency(ConnectionContextMySQL ctx, long? idCurrency)
        {
			List<Currency> list = new List<Currency>();

            var command = SqlHelper.SelectCurrencyCommand(ctx, idCurrency);
		...
		internal static List<YieldCurveFamily> GetYieldCurveFamily(ConnectionContextMySQL ctx, long? idYcFamily, long? idCcy)
        {
			List<YieldCurveFamily> list = new List<YieldCurveFamily>();

            var command = SqlHelper.SelectYieldCurveFamilyCommand(ctx, idYcFamily, idCcy);
		...
		internal static List<YieldCurveData> GetYieldCurveData(ConnectionContextMySQL ctx,
                                                                        long? idYc)
        {
            List<YieldCurveData> ycDesc = new List<YieldCurveData>();

            var command = SqlHelper.SelectYieldCurveDataCommand(ctx, idYc);
            ...
            using (var reader = command.ExecuteReader())
            {
                while (reader.Read())
                {
                    var ycs = new YieldCurveSetting
                    {
                        ...
                        SpreadSize = reader.IsDBNull(reader.GetOrdinal("SpreadSize")) ? -1 : reader.GetDouble(reader.GetOrdinal("SpreadSize")),
		...
		internal static List<EntryPoint> GetYieldCurveEntryData(ConnectionContextMySQL ctx, long? idYc, DateTime? settlementDate)
		{
			List<EntryPoint> outEntryPointList = new List<EntryPoint>();

			var command = SqlHelper.SelectYieldCurveEntryDataCommand(ctx, idYc, settlementDate);
			
			using (var reader = command.ExecuteReader())
            {
                //Dictionary<long, HistoricValue> entryPointDic = new Dictionary<long, HistoricValue>();
                ...
                while (reader.Read())
                {
					...
					d.Instrument = (d.Type == "bond"
									? (Instrument)GetBond(reader.GetInt32(reader.GetOrdinal("RateId")))
									: (Instrument)GetRate(reader.GetInt32(reader.GetOrdinal("RateId")))
									);
					...
					/*
                    //long tmp = reader.GetInt32(reader.GetOrdinal("YcEntryId"));
                    entryPointDic[d.Id] = new HistoricValue
                    {
                        Value = reader.GetDouble(reader.GetOrdinal("Value")),
                        Date = reader.GetDateTime(reader.GetOrdinal("Date"))
                    };
				...
				/*
                foreach (EntryPoint y in EntryDataList)
                {
                    y.epValue = entryPointDic[y.Id];
                }
				*/
		...
		internal static List<EntryPointHistory> GetYieldCurveEntryDataHistory(ConnectionContextMySQL ctx,
                                                                      long? idYc,
                                                                      DateTime? settlementDate)
        {
            List<EntryPointHistory> outEntryPointHistoryList = new List<EntryPointHistory>();

			var command = SqlHelper.SelectYieldCurveEntryDataHistoryCommand(ctx, idYc, null);
...
#region ---------------------------All  Rates  --------------------------------------------
		...
		public static Rate GetRate(long? idRate)
		{
			if (idRate != null && Repository.RateDic.ContainsKey(idRate.Value))
				return Repository.RateDic[idRate.Value];

			var ctx = new ConnectionContextMySQL();
			return GetRates(ctx, idRate)[0];
		}
		internal static List<Rate> GetRates(ConnectionContextMySQL ctx, long? idRate)
        {
            List<Rate> list = new List<Rate>();

            var command = SqlHelper.SelectRatesCommand(ctx, idRate);
		...
		public static Bond GetBond(long? idBond)
		{
		...
		internal static List<Bond> GetBonds(ConnectionContextMySQL ctx, long? idBond)
        {
			List<Bond> list = new List<Bond>();

            var command = SqlHelper.SelectBondsCommand(ctx, idBond);
		...
		internal static List<Calendar> GetCalendars(ConnectionContextMySQL ctx)
        {
			List<Calendar> list = new List<Calendar>();

            var command = SqlHelper.SelectCalendarsCommand(ctx);
		...
		internal static List<DayCounter> GetDayCounter(ConnectionContextMySQL ctx, long? idDC, string className)
		{
			List<DayCounter> list = new List<DayCounter>();

			var command = SqlHelper.SelectDayCounterCommand(ctx, idDC, className);
			using (var reader = command.ExecuteReader())
			{
		...
		internal static List<ExchangeRate> GetXRates(ConnectionContextMySQL ctx, long? idXRate)
        {
            List<ExchangeRate> list = new List<ExchangeRate>();

            var command = SqlHelper.SelectXRatesCommand(ctx, idXRate);
		...
		internal static List<EntryPointHistory> GetExchangeRateHistory(ConnectionContextMySQL ctx,
																				long? RateId,
																				DateTime? settlementDate)
        {
            List<EntryPointHistory> outExchangeRateHistoryList = new List<EntryPointHistory>();

            var command = SqlHelper.SelectExchangeRateHistoryCommand(ctx, RateId, null);
		...
		internal static List<YieldCurveFamily> GetInflationCurveFamily(ConnectionContextMySQL ctx, long? idICFamily, long? idCcy)
		{
			List<YieldCurveFamily> list = new List<YieldCurveFamily>();

			var command = SqlHelper.SelectInflationCurveFamilyCommand(ctx, idICFamily, idCcy);
		...
		// settings and name
		internal static List<InflationCurveData> GetInflationCurveData(ConnectionContextMySQL ctx, long? idIC)
		{
			List<InflationCurveData> ic = new List<InflationCurveData>();

			var command = SqlHelper.SelectInflationCurveDataCommand(ctx, idIC);
		...
		internal static List<InflationCurveEntryData> GetInflationCurveEntryData(ConnectionContextMySQL ctx, long? idIc, DateTime? settlementDate)
		{
			List<InflationCurveEntryData> EntryDataList = new List<InflationCurveEntryData>();

			var command = SqlHelper.SelectInflationCurveEntryDataCommand(ctx, idIc, settlementDate);
		...
		internal static List<InflationBond> GetInflationBonds(ConnectionContextMySQL ctx, long? idBond)
		//  internal static List<Bond> GetInflationBonds(ConnectionContext ctx, long? idBond)
		{
			...
			var command = SqlHelper.SelectInflationBondsCommand(ctx, idBond);
		...
		internal static List<InflationIndex> GetInflationIndex(ConnectionContextMySQL ctx, long? idIndex)
		{
			List<InflationIndex> list = new List<InflationIndex>();

			var command = SqlHelper.SelectInflationIndexCommand(ctx, idIndex);
		...
		internal static List<RateHistory> GetRateHistory(ConnectionContextMySQL ctx, DateTime? settlementDate)
		{
			List<RateHistory> RateHistoryList = new List<RateHistory>();

			var command = SqlHelper.SelectRateHistoryCommand(ctx, null, settlementDate);
		...
		internal static List<InflationRate> GetInflationRates(ConnectionContextMySQL ctx, long? idRate)
		{
			List<InflationRate> list = new List<InflationRate>();

			var command = SqlHelper.SelectInflationRatesCommand(ctx, idRate);
